...
def read_all(date_one:str, mtd_date:str, db: Session=Depends(get_db),current_user: schema.User=Depends(oauth.get_current_user) ):
    # formats= '%d, %b %Y'
...
def read(date_one:str, mtd_date:str,branch:str, db:Session=Depends(get_db),current_user: schema.User=Depends(oauth.get_current_user)):
